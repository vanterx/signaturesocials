...
This is a Next.js 14 App Router site with two content routes: `/` (home, including the merged Experiences section at `/#experiences`) and `/about`. `app/experiences/page.tsx` is a server-side `redirect('/#experiences')` kept so old links don't 404.
...
- **Each content route is a thin wrapper** (`app/page.tsx`, `app/about/page.tsx`) that renders a single top-level client component from `components/site/` (`HomePage.tsx`, `AboutPage.tsx`). All page content, section markup, and section-local CSS lives inside these components — not spread across smaller subcomponents — matching the single-big-component convention from the sibling project `electricaffair.co.nz`. `HomePage.tsx` carries the hero, genre marquee ticker, values bento, the four experience brand cards (`id="experiences"`), the "why people keep coming back" list, upcoming events, and the closing statement.
- **Styling is inline styles + one injected `<style dangerouslySetInnerHTML>` block per component** (named `*_CSS`, e.g. `HOME_CSS`, `ABOUT_CSS`; site-wide primitives like `.ss-eyebrow`, `.ss-pill`, `.ss-cta-btn`, `.ss-gradient-text` live in `app/globals.css`) — **not** Tailwind (Tailwind is not installed in this project at all, unlike Electric Affair where it's configured but unused). Everything that needs `:hover`, `::after`, `@keyframes`, or a media query goes in that string as a named class (`.ss-*` prefix); everything else is a plain inline `style={{}}` object using hardcoded brand hex values.
